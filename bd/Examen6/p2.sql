...
WHERE ORDER_ID IN (10360, 10253, 10440)
...
SELECT concat('El/la emplead@ ',employees.first_name,' ',employees.last_name,' registró ',count(orders.order_id),' pedidos') as informe from employees inner join orders ON employees.employee_id = orders.employee_id group by  employees.first_name, employees.last_name;

--En el informe anterior, sólo necesito los empleados cuyo nombre comience por M
SELECT concat('El/la emplead@ ',employees.first_name,' ',employees.last_name,' registró ',count(orders.order_id),' pedidos') as informe from employees inner join orders ON employees.employee_id = orders.employee_id WHERE employees.first_name like 'M%' group by  employees.first_name,employees.last_name ;
